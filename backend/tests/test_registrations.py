...
def _payload(seed: str, *, national_id: str | None = None, phone: str | None = None) -> dict:
    return {
        ...
        "face_sha256": _sha256(seed),
        "face_dhash": _dhash(seed),
    }
...
def test_duplicate_national_id_is_refused(client: TestClient) -> None:
    event_id = _make_event(client)
    ...
    assert detail["matched_field"] == "national_id"

    # The rejected registration must NOT be persisted.
    listed = client.get(f"/events/{event_id}/registrations").json()
    assert len(listed) == 1


def test_duplicate_phone_is_refused(client: TestClient) -> None:
    event_id = _make_event(client)
    ...
    assert resp.status_code == 409
    assert resp.json()["detail"]["matched_field"] == "phone"


def test_exact_face_match_is_refused(client: TestClient) -> None:
    event_id = _make_event(client)
    client.post(f"/events/{event_id}/registrations", json=_payload("1"))

    payload = _payload("unique-seed")
    payload["face_sha256"] = _sha256("1")
    resp = client.post(f"/events/{event_id}/registrations", json=payload)
    assert resp.status_code == 409
    assert resp.json()["detail"]["matched_field"] == "face_sha256"


def test_fuzzy_face_match_is_refused(client: TestClient) -> None:
    event_id = _make_event(client)
    base = _payload("1")
    client.post(f"/events/{event_id}/registrations", json=base)

    # Flip one bit in the dhash: hamming distance = 1, well below threshold.
    orig = int(base["face_dhash"], 16)
    flipped = orig ^ 0b1
    near = _payload("near")
    near["face_dhash"] = format(flipped, "016x")
    resp = client.post(f"/events/{event_id}/registrations", json=near)
    assert resp.status_code == 409
    assert resp.json()["detail"]["matched_field"] == "face_dhash"


def test_different_faces_not_refused(client: TestClient) -> None:
    event_id = _make_event(client)
    client.post(f"/events/{event_id}/registrations", json=_payload("1"))
...
def test_stats_and_export(client: TestClient) -> None:
    ...
    refused = client.post(f"/events/{event_id}/registrations", json=dup)
    assert refused.status_code == 409

    stats = client.get(f"/events/{event_id}/stats").json()
    assert stats == {"event_id": event_id, "total": 2, "unique": 2, "duplicates": 0}
...
def test_server_side_image_hashing(client: TestClient) -> None:
    """Client can send a raw face image and the server derives the hashes."""
    import base64
    import io
    ...
    first = client.post(
        ...
        json={
            ...
            "phone": "+254700001111",
            "face_image_b64": make_img_b64(1),
        },
    )
    assert first.status_code == 201, first.text
    body = first.json()
    assert len(body["registration"]["face_sha256"]) == 64
    assert len(body["registration"]["face_dhash"]) == 16
    assert body["duplicate"] is None

    # Second registration with a different image and different identity should succeed.
    second = client.post(
        f"/events/{event_id}/registrations",
        json={
            "full_name": "Bob",
            "national_id": "NID-bob",
            "phone": "+254700002222",
            "face_image_b64": make_img_b64(99),
        },
    )
    assert second.status_code == 201
    assert second.json()["duplicate"] is None

    # Same image as first = face_sha256 duplicate → refused.
    third = client.post(
        f"/events/{event_id}/registrations",
        json={
            "full_name": "Carol",
    ...
    assert third.json()["detail"]["matched_field"] == "face_sha256"


def test_missing_face_rejected(client: TestClient) -> None:
    event_id = _make_event(client)
    resp = client.post(
        f"/events/{event_id}/registrations",
        json={
